...
class MultiSkillContinuousPolicyNetwork(nn.Module):
    def __init__(self, stateNum, actionNum, hiddenSkill, hiddenActor, skillNum):
        super().__init__()

        hiddenSkill = [stateNum] + hiddenSkill + [skillNum]
        self.skillNet = nn.Sequential()
        for din, dout in zip(hiddenSkill[:-1], hiddenSkill[1:]):
            self.skillNet.append(nn.Linear(din, dout))
            self.skillNet.append(nn.ReLU())
        self.skillNet = self.skillNet[:-1]  # remove the last ReLU layer

        self.actionNum = actionNum
        hiddenActor = [stateNum] + hiddenActor + [actionNum*2]
        self.net = [nn.Sequential() for i in range(skillNum)]
        for net in self.net:
            for din, dout in zip(hiddenActor[:-1], hiddenActor[1:]):
                net.append(nn.Linear(din, dout))
                net.append(nn.ReLU())
            net = net[:-1]  # remove the last ReLU layer
        self.net = nn.ModuleList(self.net)

    def forward(self, state) -> tuple[torch.distributions.Categorical, torch.distributions.Normal]:
        skillProbs = F.softmax(self.skillNet(state), dim=-1)
        # print("skillProbs:", skillProbs.shape)
        logits =  torch.concatenate([net(state).unsqueeze(-2) for net in self.net], -2)
        # print("logits:", logits.shape)
        mean, std = torch.tanh(logits[:,:,:self.actionNum]), torch.clamp(torch.exp(logits[:,:,self.actionNum:]), 0.0001, 1.0)
        # print("mean:", mean.shape)
        # print("std:", std.shape)
        return Categorical(skillProbs), Normal(mean, std)
...
class MultiSkillPPOAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.policy = MultiSkillContinuousPolicyNetwork(
            self.stateNum, actionNum, hiddenSkill, hiddenActor, skillNum).apply(init_weights)
    ...
    def act(self, state: torch.Tensor, stage: Stage) -> ActionData:
        if self.device == self.trainDevice:
            self.eval()
        distSkill, distAction = self.policy(state)
        skill = distSkill.sample()
        actions = distAction.sample()
        # print("Sampled skill:", skill.shape, skill)
        # print("Sampled actions:", actions.shape, actions)
        selectedAction = actions.gather(-2, skill.view(-1,1,1).expand(-1,1,self.policy.actionNum)).squeeze(-2)
        # print("distSkill.probs:", distSkill.probs.shape, distSkill.probs)
        # print("Selected actions:", selectedAction.shape, selectedAction)
        selectedSkillProb = distSkill.probs.gather(-1, skill.view(-1,1))
        prob = distAction.log_prob(selectedAction) + torch.log(selectedSkillProb + 1e-8)
        # print("prob actions:", prob.shape, prob)
        return ActionData(
    ...
    def memorize(self, 
        ...
        # rewards = torch.clamp(rewards, min = self.minRwd)
    ...
    def learn(self, stage: Stage, nextStates: torch.Tensor) -> list[GraphPoint]:
        ...
        actions = actions.to(self.device).view(-1, 1, self.actionSpace).detach()
        probs = probs.to(self.device).view(-1, probs.size(-2), self.actionSpace).detach()
        returns = returns.view(-1, 1).detach()
        advantages = advantages.view(-1,1,1).detach()
        ...
        for epoch in range(self.trainEpoch):
            ...
            for states_, actions_, probs_, returns_, advantages_ in dataloader:
                ...
                distSkill, distAction = self.policy(states_)

                # print("distSkill.probs:", distSkill.probs.shape, distSkill.probs)
                # print("actions_:", actions_.shape, actions_)
                logProbs = distAction.log_prob(actions_.expand(-1, 2,-1)) + torch.log(distSkill.probs.unsqueeze(-1) + 1e-8)
                
                ## c.Calculate the importance ##
                # print("logProbs:", logProbs.shape)
                # print("probs_:", probs_.shape)
                R = torch.exp(logProbs - probs_)
                clipR = torch.clamp(R, 1.0-self.eps, 1.0+self.eps)

                ## d.Actor loss ##
                entropyLoss = distAction.entropy().mean() * self.entropyBeta
                # print("R:", R.shape)
                # print("advantages_:", advantages_.shape)
